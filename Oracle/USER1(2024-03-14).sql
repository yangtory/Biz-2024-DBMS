...
-- user1 사용자 권한으로 볼 수 있는 모든 테이블 보기
SELECT * FROM all_tables;
SELECT * FROM all_tables WHERE owner = 'USER1';

-- 현재 접속한 사용자 스키마에 등록된 테이블
...
문자열 : CHAR(고정문자열), VARCHAR(가변문자열)
    ...
    DB 마다 특성이 달라 SELECT 등을 수행할 때 불편함이 많다
    CHAR 타입을 사용하려면 반드시 알파벳 문자가 있는 형태로
    설계를 하는 것이 좋다
    
    문자열의 경우 VARCHAR 를 사용해도 큰 문제가 없으므로
...
유니코드 문자열 : nCHAR, nVARCHAR
    영 숫자 이외의 2byte 형 문자(한글, 아랍어, 한자 등) 을 저장하는 칼럼
    
큰 문자열 : LONG(2,000,000), CLOB(4,000,000), nCLOB(유니코드, 4,000,000)

숫자형 : NUMER, FLOAT, BINARY_DOUBLE
    NUMBER : 가변형 숫자
        NUMBER : 정수형 38자리 까지, 소수점 이하 127자리 까지
        NUMBER(5) : 정수형 최대 5자리 까지
...
전통적으로 Work Table PK 는 정수형으로 설정하고, 일련번호를 부여하는 방식을 많이 사용한다. 
    대부분의 DB 에는 AUTO INCREMENT 속성이 있어서 PK 칼럼을 설정한다
    하지만 오라클 11 이하의 버전에는 AUTO INCREMENT 속성이 아예 없다
    오라클 12c 이상에서는 속성은 추가되었지만 만드는 방법이 매우 복잡
      -->  seq NUMBER GENERATED ALWAYS AS IDENTIFY
...
-- 하지만 오라클에서는(PL*SQL) 의 SELECT 는 FROM 절이 없으면 오류가 난다
-- 일반적인 테스트 연산 등을 수행할때 DUAL Table 을 사용한다
SELECT 30 + 40 FROM DUAL;
SELECT 40 * 2 FROM DUAL;
-- sample
SELECT 30 * 40 FROM tbl_sample; -- 가지고 있는 행 개수 만큼 출력됨
