...
오라클 DB를 사용할때 제일먼저 할일이 관리자로 접속하여
TableSpace 를 만드는 일이다.

프로그램 등에서 
오라클 DB를 접속할때는 SYS, SYSTEM 등 시스템과 관련된 id 는 절대 사용하지 말자
관리자 화면에서 사용자를 생성하고, 사용자의 권한을 부여하고 접속을 한다.

오라클에서 사용하는 SCHEMA 개념을 갖는다.
사용자로 로그인을 하면, 별도의 Database 를 접속, 연결(USE)를 하지 않는다.
*/

-- myDB 라는 TableSpace 만들기
-- 물리적 저장소는 저 경로로 만들어라
...
-- myDB TableSpace 삭제하기
-- 삭제할때 모든 내용, 파일, 제약조건 등을 깨끗히 삭제하라
...
CREATE TABLE user1.tbl_user(
    sq NUMBER,
    username VARCHAR2(125), --영문, 숫자
    nick nVARCHAR2(125) --한글을 포함한 영문 외 문자
);
