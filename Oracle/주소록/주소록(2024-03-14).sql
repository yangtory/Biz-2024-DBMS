...
SELECT a_hobby1, COUNT(a_hobby1) FROM tbl_address
GROUP BY a_hobby1
...
-- 두 개의 Table 이 1:N 의 관계일때 N의 Table 에서 설정한다
...
-- 위의 SELECT(조회) 쿼리를 view_address 라는 이름의 view 생성하기
...
산술 통계를 할때 거의 필수적으로 포함되는 GROUP BY 절이 필요
*/
-- 조건 없이 전체 데이터의 개수, relation개수
...
-- 취미가 마술, 사격, 여행 인 사람들의 참여 인원수를 계산
SELECT r_hcode, h_name, COUNT(*) FROM view_address
