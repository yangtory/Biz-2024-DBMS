...
-- count 통계함수에 묶이지 않는 나머지들
SELECT io_div, COUNT(io_div) FROM tbl_iolist
GROUP BY io_div;
...
/*
데이터베이스 정규화
	RDBMS(관계형 데이터베이스 관리 시스템) 에서
		Relation DataBase Management System
	데이터의 중복을 최소화하고, 
	데이터를 구조화 하는 것
    
정규화 목표: 이상(문제점)이 있는 관계를 재구성
	잘 조직된 관계를 생성하는 것이다
...
수정, 삭제 이상은 PK 가 아닌 다른 칼럼을 조건으로
	하여 실행하는 과정에서 발생한다.
목적 : 
	이러한 이상(삽입, 수정, 삭제) 현상을 방지하기 위해서 정규화를 실행한다.
*/

-- 매입매출 테이블에서 상품정보과 거래처정보를
-- 분리하여 테이블을 구조화 하는 것

-- 상품정보를 테이블로 별도 분리하기
-- 1. 매입매출 테이블에서 상품명을 중복없이 추출
SELECT io_pname,
ROUND(AVG(io_iprice),0) AS 매입단가, -- ROUND()소수점 삭제
ROUND(AVG(io_oprice),0) AS 매출단가
FROM tbl_iolist
GROUP BY io_pname
ORDER BY io_pname; -- 이름순으로 정렬

-- 2. 매입애출 테이블의 상품명 칼럼 대신, 
-- 상품코드 칼럼으로 대체
-- 기존의 테이블을 삭제하고, 새로 작성
...
-- 5. 정규화 된 매입매출 데이터 확인하기
SELECT * FROM tbl_iolist LIMIT 10;
-- 매입매출 데이터에는 실제 보고자 하는 상품이름은 없고
-- 상품 코드 칼럼만 있는 상태
-- 이 데이터에서 매입매출 데이터와 상품데이터를 "JOIN" 하여 확인하기
SELECT * 
FROM tbl_iolist, tbl_products
WHERE io_pcode = p_code; -- io_pcode, p_code 일치하는 데이터를 한 라인에 출력

-- tbl_iolist 와 tbl_products 테이블을 join 하여
-- Projection(필요한 칼럼만 고르기) 하기, 보고싶은것만 보기
...
SELECT COUNT(*)
FROM tbl_iolist, tbl_products -- 두 개 테이블 나열하고 io_pcode, p_code 일하는 데이터 찾아서 표시 
WHERE io_pcode= p_code;
...
/*
LEFT (OUTER) JOIN
이 키워드를 기준으로 왼쪽에 있는 Table 은 무조건 다 나열해라
오른쪽의 테이블에서 ON의 조건에 맞는 데이터를 SELECT 해라
만약 SELECT 된 결과가 있으면 출력하고
없으면 NULL 로 출력해라
tbl_iolist 에는 있는데 tbl_products 에 데이터가 없으면
null 로 출력해
tbl_iolist 와 tbl_products 간에 
"참조 관계"가 완전한지를 검증하는 코드 (참조 무결성을 검증하는 코드)
*/
...
-- 거래처 정보의 정규화
-- 매입 매출 데이터에서 거래처 정보를 분리하기
-- 1. 매입매출 데이터에서 거래처명, 대표자명을
-- 중복되지 않는 데이터로 추출하기
SELECT io_dname, io_dceo
FROM tbl_iolist
GROUP BY io_dname, io_dceo -- 거래처명이 같으면 ceo 명으로 정리해
ORDER BY io_dname;
